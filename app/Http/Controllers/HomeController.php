...
class HomeController extends Controller
{
    ...
    public function main(){
        $data['Block_Blog']=BlogEtcPost::orderBy('id','desc')->paginate(3);
    ...
    public function search_job($keyword = '')
    {
        ...
        if(Input::has('keyword'))
        {
            ...
            $data = ProfessorData::where('sub_disciplines1', 'LIKE', "%{$keyword}%")->orWhere('sub_disciplines2', 'LIKE', "%{$keyword}%")->orWhere('sub_disciplines3', 'LIKE', "%{$keyword}%")->orWhere('sub_disciplines4', 'LIKE', "%{$keyword}%")->orWhere('disciplines', 'LIKE', "%{$keyword}%")->get();
        }
        
        return view('user.search-job',compact('data'));
    }
